          ...
          <AnimatePresence mode="wait">
            ...
            {step === 1 && (
              <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                ...
                {/* مثال لزر التخطي الذي يمكنك إضافته تحت زر Continue */}
                <div className="mt-4 text-center">
                  <button 
                    onClick={() => router.push("/hub")} // ينقلك مباشرة للـ Student Hub
                    className="text-xs font-bold text-slate-400 hover:text-indigo-500 transition-colors cursor-pointer uppercase tracking-widest"
                  >
                    Skip for now
                  </button>
                </div>
